...
import { Calculator, CheckCircle, AlertTriangle, Send, Loader2, Plus, Trash2 } from 'lucide-react';
...
export const EmitNfeModal: React.FC<EmitNfeModalProps> = ({
  sale,
  onClose,
  formatCPF
}) => {
  const [step, setStep] = useState<'INPUT' | 'CALCULATING' | 'RESULT' | 'TRANSMITTING' | 'SUCCESS'>('INPUT');
  const [error, setError] = useState<string | null>(null);
  
  const [rows, setRows] = useState<TaxInput[]>([
    {
      id: '1',
      produto: sale.productName || 'Produto Genérico',
      atividade: 'Comercio',
      custoCompra: '',
      ipi: '',
      frete: '',
  ...
  const updateRow = (id: string, field: keyof TaxInput, value: string) => {
    ...
    <div className="fixed inset-0 z-[130] flex items-center justify-center bg-black/60 backdrop-blur-xs p-4 overflow-y-auto animate-fade-in">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-7xl overflow-hidden border border-slate-100 flex flex-col max-h-[95vh]">
        ...
        <div className="flex justify-between items-center border-b p-5 shrink-0 bg-slate-50">
          <div>
            <h3 className="text-lg font-black text-slate-900 font-display uppercase tracking-wider flex items-center gap-2">
              ...
              <span>Precificação e Emissão de NF-e</span>
            </h3>
            <p className="text-xs text-slate-500 mt-1">Simulador estilo planilha (Gross Up / Markup Divisor)</p>
          </div>
        ...
        <div className="p-5 overflow-y-auto flex-1 flex flex-col">
          {error && (
            <div className="bg-rose-50 border border-rose-200 text-rose-700 p-3 rounded-lg text-xs font-bold mb-4 shrink-0">
              {error}
            </div>
          )}
          ...
          {step === 'INPUT' && (
            <div className="flex-1 flex flex-col min-h-0">
              <div className="bg-blue-50 border border-blue-100 p-3 rounded-lg flex gap-3 text-xs text-blue-800 mb-4 shrink-0">
                <AlertTriangle className="w-4 h-4 shrink-0 text-blue-600" />
                <p>Preencha os valores da compra para calcularmos o custo líquido e o preço final de venda (Baseado no Simples Nacional - BA).</p>
              </div>
                ...
                <table className="w-full text-left text-xs whitespace-nowrap min-w-[1200px]">
                  <thead className="bg-slate-100 text-slate-600 font-bold sticky top-0 z-10 shadow-sm">
                    <tr>
                      ...
                      <th className="p-3 w-24 text-right">IPI (R$)</th>
                      ...
                      <th className="p-3 w-24 text-right">Antecip. (R$)</th>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {rows.map((row, index) => (
                      <tr key={row.id} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="p-2">
                          <input type="number" value={row.custoCompra} onChange={(e) => updateRow(row.id, 'custoCompra', e.target.value)} className="w-full border-slate-200 rounded text-xs px-2 py-1.5 text-right font-mono" placeholder="0.00" />
                        </td>
                        <td className="p-2">
                        ...
                        <td className="p-2 text-center">
                          <button onClick={() => removeRow(row.id)} disabled={rows.length === 1} className="p-1.5 text-slate-400 hover:text-rose-500 hover:bg-rose-50 rounded disabled:opacity-30">
              ...
              <div className="mt-3 shrink-0">
                <button onClick={addRow} className="text-indigo-600 hover:text-indigo-800 text-xs font-bold flex items-center gap-1 px-2 py-1 rounded hover:bg-indigo-50">
          ...
          {step === 'TRANSMITTING' && (
            <div className="flex flex-col items-center justify-center py-20 space-y-4 text-center">
              ...
              <h4 className="font-bold text-sm text-slate-900">Transmitindo para a SEFAZ...</h4>
            </div>
          )}
          ...
          {step === 'SUCCESS' && (
            <div className="space-y-4 text-center py-12">
              ...
              <h4 className="font-black text-2xl text-slate-900">Nota Fiscal Emitida!</h4>
              <p className="text-sm text-slate-500">A NF-e foi autorizada com todos os {results.length} itens inclusos.</p>
            </div>
          )}
        </div>
        ...
        <div className="p-4 border-t bg-slate-50 shrink-0 flex justify-end gap-3">
          {step === 'RESULT' && (
            <button onClick={() => setStep('INPUT')} className="px-4 py-2 border border-slate-300 text-slate-700 rounded-lg text-sm font-bold bg-white hover:bg-slate-50">
              Voltar e Editar
            </button>
          )}
          
          <button onClick={onClose} className="px-4 py-2 border border-transparent text-slate-500 hover:text-slate-700 rounded-lg text-sm font-bold">
            {step === 'SUCCESS' ? 'Fechar Janela' : 'Cancelar'}
          </button>
          
          {step === 'INPUT' && (
            <button onClick={handleCalculate} className="px-6 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-sm font-black flex items-center gap-2 shadow-sm">
              <Calculator className="w-4 h-4" /> Calcular Tabela
            </button>
          )}

          {step === 'RESULT' && (
            <button onClick={handleTransmit} className="px-6 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-sm font-black flex items-center gap-2 shadow-sm">
              <Send className="w-4 h-4" /> Emitir NF-e (Focus)
            </button>
          )}
        </div>
